...
//!
//! Mesmo motor do LocalMedia (a suíte compartilha a pegada, não reimplementa):
//! os ARGUMENTOS de cada conversão são montados no front (src/lib/formats.ts,
//! puro e unit-testado); aqui só resolvemos o binário, injetamos
//! `-progress pipe:1` (progresso ESTRUTURADO — nunca regex no log humano),
...
mod tests {
    ...
    fn progresso_estruturado() {
        assert_eq!(parse_progress_line("out_time_ms=1500000"), Some(("t", "1500000".to_string())));
        assert_eq!(parse_progress_line("speed=2.31x"), Some(("speed", "2.31x".to_string())));
        assert_eq!(parse_progress_line("progress=continue"), Some(("progress", "continue".to_string())));
